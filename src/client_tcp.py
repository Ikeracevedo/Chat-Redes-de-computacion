...
def main():
    if len(sys.argv) < 3:
        print("Uso: python src/client_tcp.py <IP_SERVIDOR> <PUERTO>")
        print("Ej:  python src/client_tcp.py 127.0.0.1 5555")
        sys.exit(1)
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        try:
            while True:
                ...
                # /msg primero porque crea un ChatMessage con 'to'
                if line.startswith("/msg "):
                    parts = line.split(" ", 2)
                    if len(parts) < 3:
                        print("Uso: /msg <dest> <mensaje>  (dest puede ser alias o IP)")
                        continue
                    dest, text = parts[1], parts[2]
                    chat = ChatMessage(
                        from_id="",   # el server mostrará tu alias/IP visible
                        msg=text,
                        ts=now_ts(),
                        mid=gen_id(),
                        cmd=None,
                ...
                if line.startswith("/"):
                    if line == "/who":
                        ...
                        print("Comandos: /who, /nick <alias>, /quit, /msg <dest> <texto>")
                        continue
